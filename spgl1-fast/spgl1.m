...
function [x,r,g,info] = spgl1(fk, Na, E1, E2, mL, Mr, EH1, EH2, nL, nL1, dt, Nr,  ...
...
info.timeMatProd1 = timeMatProd1;
...
printf(' %-20s:  %6i %6s %-20s:  %6.1f   %6.1f\n',...
    'Newton iterations',nNewton,'','Mat-vec time (secs)',timeMatProd1, timeMatProd);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function z = Aprod(x,mode)
        ...
        if mode == 1
            ...
            [f_taur,f_taui] = AXnufft(double(real(x(:))), double(imag(x(:))),...
                double(E1(:)), double(E2(:)), double(mL(:)), double(fk(:)), [Mr, 3, Na]);

            z = f_taur + 1j * f_taui;
            timeMatProd1 = timeMatProd1 + (toc - tStart);
        elseif mode == 2
            nProdAt = nProdAt + 1;
            ...
            [f_taur,f_taui] = AHxintp(double(real(x(:))), double(imag(x(:))), ...
